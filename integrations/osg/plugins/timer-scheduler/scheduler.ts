...
import { nextCronTriggerAt, parseCronExpression } from "./cron.js";
...
async function fireTimer(context: PluginContext, timerID: string): Promise<void> {
  ...
  try {
    ...
    const result = await context.osg.addPrompt({
    ...
    if (!result.ok) {
      throw new Error(result.error || "timer prompt submit failed");
    }
  } catch (error) {
