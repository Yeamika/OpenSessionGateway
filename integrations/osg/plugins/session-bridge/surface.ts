...
import { DELETE_MAILBOX_ITEM_TOOL, createDeleteMailboxItemToolHandler } from "./tools/DeleteMailboxItem.js";
...
export function createSessionBridgeMcpPlugin(services: SessionBridgeServices): McpPlugin {
  ...
  const handleDeleteMailboxItemTool = createDeleteMailboxItemToolHandler(services);
  ...
  async function handleSessionBridgeRpc(body: unknown, req: NextRequest) {
    ...
    if (method === "tools/call") {
      ...
      try {
        ...
        if (toolName === "DeleteMailboxItem") {
          return NextResponse.json(successResult(id, textResult(await handleDeleteMailboxItemTool(toolArgs, runtimeID))));
        }
