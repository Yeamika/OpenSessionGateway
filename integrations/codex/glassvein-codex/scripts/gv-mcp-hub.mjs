...
import { ensureRouteForCaller, startGvReceiver } from "./gv-codex-app-server-bridge.mjs"
...
startGvReceiver(readCaller).catch((error) => {
  console.error(`[gv-mcp] receiver disabled: ${errorMessage(error)}`)
})
...
async function callTool(params) {
  ...
  if (!caller.sessionID) {
    throw new Error("No Codex session has been captured yet; submit one prompt first so GlassVein can bind ownership.")
  }
  await ensureRouteForCaller(caller)

  const userArgs = params.arguments && typeof params.arguments === "object" ? params.arguments : {}
