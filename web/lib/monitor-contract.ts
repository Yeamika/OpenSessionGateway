export type MonitorRuntimeStatus = "online" | "stale" | "offline";
...
const RUNTIME_STATUSES: MonitorRuntimeStatus[] = ["online", "stale", "offline"];
...
export function sortMonitorClients(data: MonitorClient[]): MonitorClient[] {
  return [...data].sort((a, b) => {
    ...
    const workspace = (a.workspace || "").localeCompare(b.workspace || "");
...
export function readMonitorClient(value: unknown): MonitorClient | null {
  ...
  const workspace = readNullableString(value.workspace);
