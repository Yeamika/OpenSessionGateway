use crate::{
    api, config::ConfigStore, gv_client::GvClient, mcp, session_tools::SessionToolServices,
    state::SharedState,
};
...
pub async fn serve(
    ...
    loop {
        ...
        tokio::spawn(async move {
            if let Err(error) = handle(stream, state, gv, session_tools, config).await {
...
async fn handle(
    ...
    let response = route(method, path, body, &state, &gv, &session_tools, &config).await;
...
async fn route(
    ...
    match (method, path) {
        ...
        ("POST", "/mcp") => {
            json_response(mcp::handle(state, gv, session_tools, config, body).await)
        }
        _ => Ok((
            404,
            "application/json",
