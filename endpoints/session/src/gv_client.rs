...
impl GvClient {
    ...
    pub async fn send(&self, link_type: &str, subtype: &str, payload: Value) -> Result<Uuid> {
        ...
        tokio::select! {
            Some(command) = rx.recv() => match command {
                ClientCommand::Send(envelope) => {
                    ...
                    let frame = serde_json::to_string(&LinkMessage::Envelope(envelope.clone()))?;
                    writer.send(Message::Text(frame.into())).await?;
                    state.log(label, envelope.payload.clone()).await;
                }
...
async fn handle_text(state: &SharedState, text: &str) -> Result<()> {
    ...
    match msg {
        LinkMessage::Envelope(envelope) => {
            if envelope.link_type == "upload" && envelope.subtype == "session_update" {
                state.session_update(envelope.payload).await;
            } else if envelope.link_type == "response" {
                state
                    .log(format!("response {}", envelope.subtype), envelope.payload)
                    .await;
            }
        }
