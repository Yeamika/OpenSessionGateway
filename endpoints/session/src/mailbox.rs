...
impl MailboxStore {
        ...
        let list: Vec<Value> = rows
            ...
            .map(|row| {
                json!({
                    ...
                    "InfoType": info_type(row),
                    "hasRead": row.has_read,
                    "title": row.title,
                    "times": row.created_at_ms,
                })
            })
            .collect();
    ...
    pub async fn send(&self, args: &Value) -> Value {
        ...
        let executor_session_id = required_string(args, "ExecutorSessionID", "session-endpoint");
        ...
        let row = MailboxItem {
            ...
            sender_runtime_id: if executor_runtime_id.is_empty() {
                "session-endpoint".into()
            } else {
                executor_runtime_id
            },
            sender_session_id: executor_session_id,
            sender_session_title: required_string(args, "senderSessionTitle", "Session Endpoint"),
    ...
    pub async fn reply(&self, args: &Value) -> Value {
        ...
        let executor_runtime = executor_runtime(args);
        let mut buckets = self.0.lock().await;
        let mut reply: Option<(String, MailboxItem)> = None;
        for (session, rows) in buckets.iter_mut() {
            if !executor_session.is_empty() && session != &executor_session {
                continue;
            }
...
fn make_reply(original: &MailboxItem, message: &str, args: &Value) -> MailboxItem {
    ...
    MailboxItem {
        ...
        sender_session_id: required_string(args, "ExecutorSessionID", "session-endpoint"),
        sender_session_title: required_string(args, "senderSessionTitle", "Session Endpoint"),
