use crate::{
    ...
    config::ConfigStore,
    gv_client::GvClient,
    session_bridge::{bridge_tool_names, BridgeServices},
    state::SharedState,
};
...
pub async fn handle(
    ...
    match handle_result(state, gv, bridge, config, body).await {
...
async fn handle_result(
    ...
    let result = match request.method.as_str() {
        "initialize" => json!({ "name": "session-endpoint", "version": env!("CARGO_PKG_VERSION") }),
        "tools/list" => list_tools(),
        "tools/call" => call_tool(state, gv, bridge, config, request.params).await?,
...
async fn call_tool(
    ...
    match name {
        ...
        bridge_name => {
            if let Some(result) = bridge.call(bridge_name, args, state).await? {
                Ok(result)
            } else {
                bail!("unknown tool '{bridge_name}'")
            }
        }
    }
}
...
fn list_tools() -> Value {
    ...
    tools.extend(bridge_tool_names().iter().map(|name| {
        json!({ "name": name, "inputSchema": crate::session_bridge::bridge_tool_schema(name) })
    }));
    json!({ "tools": tools })
}
