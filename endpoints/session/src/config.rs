...
impl ConfigStore {
    pub fn new(path: Option<PathBuf>, listen_override: Option<SocketAddr>) -> Self {
        Self { path: Arc::new(Mutex::new(path)), listen_override }
    }
    ...
    pub async fn reload(&self, state: &SharedState) -> Result<Value> {
        let config = self.read_config().await?;
        state.set_runtime_config(config.clone()).await;
        state.log("config_reloaded", serde_json::to_value(&config)?).await;
...
fn runtime_config(file: FileConfig, listen_override: Option<SocketAddr>) -> RuntimeConfig {
    ...
    let endpoint = EndpointConfig {
        router_url: file.router_url.unwrap_or_else(|| "ws://127.0.0.1:7200".into()),
        node_id: file.node_id.unwrap_or_else(|| "session-endpoint".into()),
        source: address(file.source, "surface", Some("session-endpoint"), None),
...
fn address(
    ...
    SessionAddress::new(
        ...
        value.runtime.or_else(|| default_runtime.map(str::to_string)),
        value.session.or_else(|| default_session.map(str::to_string)),
    )
}
