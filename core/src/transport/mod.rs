...
//! `InMemoryTransport` for testing without real I/O.
//!
//! ## Pingora transport
//!
//! When the `pingora-transport` feature is enabled, the [`pingora`] submodule
//! provides a `PingoraTransport` and `PingoraServerApp` that use Pingora's
//! server runtime for TCP/TLS accept, with `tokio-tungstenite` as the
//! WebSocket codec layer.
//!
//! ## Other reserved slots
//!
//! - **TCP transport** — adapter over `tokio::net::TcpStream`.
...
/// The forward engine sends and receives `LinkMessage` values through this
/// trait without knowing the underlying transport.
#[async_trait]
pub trait Transport: Send + Sync + 'static {
    /// Send a link message to the remote end.
    async fn send(&self, message: LinkMessage) -> Result<()>;

    /// Receive the next link message. Returns `Ok(None)` when the remote
...
pub struct InMemoryTransport {
    incoming: Arc<tokio::sync::Mutex<mpsc::UnboundedReceiver<LinkMessage>>>,
    outgoing: mpsc::UnboundedSender<LinkMessage>,
...
/// A thread-safe map of neighbor ID → transport.
///
/// Used by the forward engine to look up which transport to use when
/// executing a forward decision.
...
impl TransportMap {
    ...
    /// Get the lexicographically smallest ID and its transport.
    /// Used for deterministic fallback routing.
    pub async fn smallest(&self) -> Option<(String, Arc<dyn Transport>)> {
...
mod tests {
    ...
    async fn in_memory_pair_sends_and_receives() {
        ...
        let msg = LinkMessage::Announce {
            address: SessionAddress::new("d1", None, None),
            distance: 0,
        };

        left.send(msg.clone()).await.unwrap();

        let received = right.recv().await.unwrap().expect("should receive");
        match received {
            LinkMessage::Announce { address, .. } => assert_eq!(address.domain, "d1"),
    ...
    async fn in_memory_bidirectional() {
        ...
        let msg_a = LinkMessage::Ping;
        let msg_b = LinkMessage::Pong;

        left.send(msg_a).await.unwrap();
        right.send(msg_b).await.unwrap();

        let recv_b = left.recv().await.unwrap().unwrap();
        assert!(matches!(recv_b, LinkMessage::Pong));

        let recv_a = right.recv().await.unwrap().unwrap();
        assert!(matches!(recv_a, LinkMessage::Ping));
    }
    ...
    async fn transport_map_insert_get_remove() {
        ...
        map.insert("n1".into(), Arc::new(t1)).await;
        map.insert("n2".into(), Arc::new(t2)).await;

        assert_eq!(map.len().await, 2);
        assert!(map.get("n1").await.is_some());
        assert!(map.get("n3").await.is_none());

        assert!(map.remove("n1").await);
        assert_eq!(map.len().await, 1);
    }

    #[tokio::test]
    async fn transport_map_smallest_deterministic() {
        let map = TransportMap::new();
        let (t_z, _) = InMemoryTransport::pair();
        let (t_a, _) = InMemoryTransport::pair();

        map.insert("neighbor-z".into(), Arc::new(t_z)).await;
        map.insert("neighbor-a".into(), Arc::new(t_a)).await;

        let (id, _) = map.smallest().await.unwrap();
        assert_eq!(id, "neighbor-a");
    }
}
