export type { ListSessionResponsePayload as SessionListResponse, SessionListItem } from "@opensessiongateway/protocol-library/ws-protocol/SessionList.js";
import type { ListSessionResponsePayload as SessionListResponse, SessionListItem } from "@opensessiongateway/protocol-library/ws-protocol/SessionList.js";
import { createListSessionRequestPayload } from "@opensessiongateway/protocol-library/ws-protocol/SessionList.js";
...
export async function readCurrentSessionList(
  ...
  const result = await ctx?.client?.session?.list?.({}).catch(() => null);
